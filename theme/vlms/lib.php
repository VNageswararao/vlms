 ...
 * @return string
 */
function theme_vlms_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_vlms_set_headerimg($theme);

    $scss .= theme_vlms_set_topfooterimg($theme);

    $scss .= theme_vlms_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_vlms_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_vlms_set_topfooterimg($theme) {
    global $OUTPUT;
    $headercss='';
 ...
 * @return string
 */
function theme_vlms_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.vlms-login #page-wrapper #page {background-image: url('$loginbgimg');}";
 ...
 * @return string
 */
function theme_vlms_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_vlms', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_vlms and not theme_boost (see the line above).
    ...
    // vlms scss.
    $vlmsvariables = file_get_contents($CFG->dirroot . '/theme/vlms/scss/vlms/_variables.scss');
    $vlms = file_get_contents($CFG->dirroot . '/theme/vlms/scss/vlms.scss');

    // Combine them together.
    $allscss = $vlmsvariables . "\n" . $scss . "\n" . $vlms;
 ...
 * @return string
 */
function theme_vlms_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_vlms_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('vlms');
 ...
 * @return string
 */
function theme_vlms_get_setting($setting, $format = false) {
    $theme = theme_config::load('vlms');
...
/**
 * Extend the vlms navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_vlms_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_vlms_add_certificatesmenuitem($flatnav);

    theme_vlms_delete_menuitems($flatnav);

    theme_vlms_add_coursesections_to_navigation($flatnav);
}
 ...
 *
 */
function theme_vlms_add_certificatesmenuitem(\flat_navigation $flatnav) {
    global $COURSE;

    try {
        if (!theme_vlms_has_certificates_plugin()) {
            return;
        }

        $actionurl = new \moodle_url('/theme/vlms/certificates.php');

        // Course page.
        if ($COURSE->id > 1) {
            $parentitem = $flatnav->find('competencies', \navigation_node::TYPE_SETTING);

            $actionurl = new \moodle_url('/theme/vlms/certificates.php', ['id' => $COURSE->id]);
        }
        ...
        if (!is_null($parentitem->parent)) {
            $certificatesitemoptions = [
                'action' => $actionurl,
                'text' => get_string('certificates', 'theme_vlms'),
                'shorttext' => get_string('certificates', 'theme_vlms'),
 ...
 * @param flat_navigation $flatnav
 */
function theme_vlms_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_vlms_add_coursesections_to_navigation(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_vlms'),
            'shorttext' => get_string('coursesections', 'theme_vlms'),
 ...
 * @return bool
 */
function theme_vlms_has_certificates_plugin() {
